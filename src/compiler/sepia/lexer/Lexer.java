...
public class Lexer {
	...
	public void nextToken() {
		...
		if (in[k] == '/' && in[k + 1] == '/') {
			...
			if (Character.isLetter(in[k]) || in[k] == '_') {
				 ...
				 * if (in[k] == ':') { ident = ident + ":"; k++; token =
				 * Token.IDENTCOLON; symbol = new SymbolIdent(token, ident,
				 * startLine, lineNumber, getColumn()); } else {
				 */
	...
	/**
	 * name is "source" or "text". This method is called when we have source<<*
	 * .... *>> or text[<+ .... +>]
	 * 
	...
	private Symbol getMetaobjectText(String name) {
		...
		if (!isSequenceChar(in[k])) {
			...
			if (text != null) {
				symbol = new SymbolCTMOCall(token = Token.CTMOCALL, name,
						fromCharArray(leftCharArray),
						fromCharArray(rightCharArray), text, startLine,
						lineNumber, getColumn());
	...
	/**
	 * Assumes that in[k] is in the first character after " in a string. That
	 * is, in "olá" in[k] must be 'o'. This method does not consider escape
	 * characters --- it should be called when the string starts with @" as in
	 * 
	 * @"c:\t\f" This string has six characters, including two '\\'.
	 * 
	 * @return
	 */
	...
	/**
	 * returns true if the parameter can be part of a sequence that delimits a
	 * literal object or a metaobject.
	 * 
	 * @param ch
	 * @return
	 */
	...
	/**
	 * Janaína: change this method. As it is, it prints a message in the
	 * standard output, messing with the IDE.
	 * 
	 * @param message
	 */
	...
	/**
	 * return a string with the contents of charArray (which may be '\0'
	 * terminated).
	 * 
	 * @param charArray
	 * @return
	...
	/**
	 * gets a left symbol sequence of a literal object or metaobjec as input and
	 * returns the right symbol sequence: (# returns #) <(* returns *)>
	 * 
	 * @param leftCharArray
	 *            should be a sequence of char´s terminated with '\0'
	 * @param size
	 * @return
	 */
	private char[] getRightSymbolSeq(char[] leftCharArray, int size) {
		...
		/*
		 * realSize makes this method works whether leftCharArray terminates
		 * with '\0' or not.
		 */
		int realSize = size;
		if (leftCharArray[size - 1] == '\0')
			--realSize;
	...
	/**
	 * the current character, in[k], is the first one of a metaobject call or
	 * literal object. in[k] would be '1' in the examples below
	 * 
	 * @version<<+1+>> ($$1 > 0 $$) This method returns the text between the
	 *                 current character (including it) and the sequence of
	 *                 symbols rightCharArray.
	 * 
